...
fn probability_fields() -> Vec<View> {
    ...
    let handicraft = view_model.signal.get_clone().handicraft.map(|x| x.to_string());
    let enhance_mastery = view_model.signal.get_clone().enhance_mastery.map(|x| x.to_string());
    let upgrade_salvation = view_model.signal.get_clone().upgrade_salvation.map(|x| x.to_string());
...
fn equipment_fields() -> Vec<View> {
    ...
    let equipment_level = view_model.signal.get_clone().equipment_level.map(|x| x.to_string());
...
fn price_fields() -> Vec<View> {
    ...
    let trace_price = view_model.signal.get_clone().trace_price.map(|x| x.to_string());
